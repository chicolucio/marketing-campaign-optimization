...
# and `pip install marketing_case` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "marketing_case", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n marketing_case -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "marketing_case", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "marketing_case", "YOUR_CONSOLE_SCRIPT"]
